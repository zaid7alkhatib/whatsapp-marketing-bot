...
exports.getBusinessPartners = getBusinessPartners;
...
const business_partner_model_1 = require("./business-partner.model");
...
function parseCreateBody(body) {
    if (!isNonEmptyString(body.type) || !business_partner_types_1.BUSINESS_PARTNER_TYPES.includes(body.type)) {
        return {
            isValid: false,
            message: `Field 'type' must be one of: ${business_partner_types_1.BUSINESS_PARTNER_TYPES.join(", ")}.`,
        };
    }
    if (!isNonEmptyString(body.subtype) ||
        !business_partner_types_1.BUSINESS_PARTNER_SUBTYPES.includes(body.subtype)) {
        return {
            isValid: false,
            message: `Field 'subtype' must be one of: ${business_partner_types_1.BUSINESS_PARTNER_SUBTYPES.join(", ")}.`,
        };
    }
    const status = body.status ?? "active";
    if (!isNonEmptyString(status) || !business_partner_types_1.BUSINESS_PARTNER_STATUSES.includes(status)) {
        return {
            isValid: false,
            message: `Field 'status' must be one of: ${business_partner_types_1.BUSINESS_PARTNER_STATUSES.join(", ")}.`,
        };
    }
    ...
    if (body.identifiers?.insuranceNumber !== undefined &&
    ...
    if (body.identifiers?.patientNumber !== undefined &&
    ...
    const cleanedTags = Array.isArray(body.tags)
        ...
        : undefined;
    const personalInfo = body.personalInfo && (parsedDateOfBirth.date || body.personalInfo.gender)
        ? {
            dateOfBirth: parsedDateOfBirth.date,
            gender: body.personalInfo.gender?.trim(),
        }
        : undefined;
    const contactInfo = body.contactInfo && (body.contactInfo.phone || body.contactInfo.email)
        ? {
...
async function getBusinessPartners(_req, res, next) {
    try {
        const businessPartners = await business_partner_model_1.BusinessPartnerModel.find().sort({ createdAt: -1 }).lean();
        res.status(200).json({
            success: true,
...
async function getBusinessPartnerById(req, res, next) {
    try {
        const { id } = req.params;
        ...
        const businessPartner = await business_partner_model_1.BusinessPartnerModel.findById(id).lean();
        if (!businessPartner) {
            res.status(404).json({
                success: false,
...
async function createBusinessPartner(req, res, next) {
    try {
        ...
        const businessPartner = await business_partner_model_1.BusinessPartnerModel.create(parsed.data);
        res.status(201).json({
            success: true,
            data: businessPartner,
        });
...
async function updateBusinessPartner(req, res, next) {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.isValidObjectId(id)) {
            res.status(400).json({
        ...
        const existingBusinessPartner = await business_partner_model_1.BusinessPartnerModel.findById(id);
